...
$(document).ready(function() {
	...
	$('.comp_sel').click(function(){
		$('.pitem_add').attr('data-comp', $(this).attr('data-id'));
	})
	...
	$('.pitem_add').on('click', function() {
		if ($('.pitem_add').attr('data-comp')) {
			$.ajax({
				url: "/products/item/get.php?pitem_add",
				type: "POST",
				dataType: "html",
				data: ({
					id: $('.pitem_add').attr('data-id'),
					quantity: $('.pitem_quantity').attr('data-val'), 
					comp: $('.pitem_add').attr('data-comp'),
				}),
				beforeSend: function(){ mess('Отправка..') },
				error: function(data){ },
				success: function(data){
	...
	$('html').on('input', '.view_updq_qn', function () {
		...
		if (btn.attr('data-sel') != 1) mess('Введите свой данный')
			...
			$.ajax({
				url: "/products/item/get.php?pitem_qn",
				type: "POST",
				dataType: "html",
				data: ({ 
